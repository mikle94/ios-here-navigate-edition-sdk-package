...
function direction.get_highway_exit_information(order, trigger, exit_number, exit_name, road_class, distance_in_meters, direction_information_limit, highway_exit_rules)
    ...
    local exit_information = {['exit_number'] = exit_number, ['exit_name'] = exit_name}
...
function direction.get_exit_key(exit_number, exit_name)
    local exit_array = {}
    if exit_number then exit_array[#exit_array + 1] = 'exit_number' end
