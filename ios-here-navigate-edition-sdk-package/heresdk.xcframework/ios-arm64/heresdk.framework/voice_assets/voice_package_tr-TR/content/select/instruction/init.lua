...
function instruction.get_additional_info(additional_content)
    ...
    if utils.inside('exit_number', utils.keys(additional_content)) or
       utils.inside('exit_name', utils.keys(additional_content)) then
...
function instruction.get_use_exit_purpose(order, trigger, leaving_highway, exit_number, exit_name, use_exit_purpose_rules)
    if not use_exit_purpose_rules then return nil end
    local exit_key = direction.get_exit_key(exit_number, exit_name)
...
function instruction.select_highway_turn(message, rules)
    ...
    local highway_exit_information = direction.get_highway_exit_information(message.order, message.trigger, message.exit_number, message.exit_name,
    ...
    local use_exit_purpose = instruction.get_use_exit_purpose(message.order, message.trigger, message.leaving_highway, message.exit_number,
